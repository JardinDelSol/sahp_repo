...
def eval_sahp(
    ...
    for i_batch in loop_range:
        (
            ...
            batch_size, i_batch, model, seq_lengths, seq_times, seq_types, rnn=False
        )
        batch_seq_types = batch_seq_types[:, 1:]

        masked_seq_types = MaskBatch(
            batch_seq_types, pad=model.process_dim, device=device
        )  # exclude the first added event
...
def train_eval_sahp(params):
    ...
    max_sequence_length = max(
        train_seq_lengths[0], dev_seq_lengths[0], test_seq_lengths[0]
    )
    ...
    model_opt = atten_optimizer.NoamOpt(
        args.d_model, 1, 100, initial_lr=args.lr, optimizer=optimizer
    )
    ...
    for epoch in range(epoch_num):
        ...
        for i_batch in tr_loop_range:
            ...
            if i_batch % 50 == 0:
                ...
                print(
                    "Epoch {} Batch {}: Negative Log-Likelihood per event: {:5f} nats".format(
                        epoch, i_batch, loss.item() / batch_event_num
                    )
                )
        ...
        print(
            "---\nEpoch.{} Training set\nTrain Negative Log-Likelihood per event: {:5f} nats\n".format(
                epoch, epoch_train_loss / train_event_num
            )
        )

        ## dev
        dev_event_num, epoch_dev_loss = eval_sahp(
            batch_size,
            de_loop_range,
            dev_seq_lengths,
            dev_seq_times,
        ...
        print(
            "Epoch.{} Devlopment set\nDev Negative Likelihood per event: {:5f} nats.\n".format(
                epoch, epoch_dev_loss / dev_event_num
            )
        )

        ## test
        test_event_num, epoch_test_loss = eval_sahp(
            batch_size,
            test_loop_range,
            test_seq_lengths,
            test_seq_times,
        ...
        print(
            "Epoch.{} Test set\nTest Negative Likelihood per event: {:5f} nats.\n".format(
                epoch, epoch_test_loss / test_event_num
            )
        )
        ...
        # prediction
        avg_rmse, types_predict_score = prediction_evaluation(
            device,
            model,
            test_seq_lengths,
            test_seq_times,
...
def prediction_evaluation(
    ...
    types_predict_score = f1_score(
        types_real, types_estimates, average="micro"
    )  # preferable in class imbalance
