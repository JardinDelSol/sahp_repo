...
class SeqGenerator:
    def __init__(self, model: nn.Module, record_intensity: bool = False):
        ...
        print(
            "Process models dim:\t{}\tHidden units:\t{}".format(
                self.process_dim, model.hidden_size
            )
        )
        self.event_times = []
        self.event_types = []
    ...
    def plot_events_and_intensity(self, model_name: str = None, debug=False):
        ...
        fig, ax = plt.subplots(1, 1, sharex="all", dpi=100, figsize=(9, 4.5))
        ax: plt.Axes
        ...
        for y, lab in zip(intens_hist.T, labels):
            ...
            ax.plot(self._plot_times, y, linewidth=0.7, label=lab)
            # pass
...
def predict_from_hidden(
    ...
    if plot:
        ...
        ax0.plot(dt_vals.numpy(), density.numpy(), linestyle="-", linewidth=0.8)
