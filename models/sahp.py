...
class SAHP(nn.Module):
    ...
    def __init__(
        ...
        self.type_emb = TypeEmbedding(
            self.input_size, d_model, padding_idx=self.process_dim
        )
        self.position_emb = BiasedPositionalEmbedding(
            d_model=d_model, max_len=max_sequence_length
        )

        self.attention = MultiHeadedAttention(h=atten_heads, d_model=self.d_model)
        self.feed_forward = PositionwiseFeedForward(
            d_model=self.d_model, d_ff=self.d_model * 4, dropout=dropout
        )
        ...
        self.start_layer = nn.Sequential(
            nn.Linear(self.d_model, self.d_model, bias=True), self.gelu
        )
        ...
        self.decay_layer = nn.Sequential(
            nn.Linear(self.d_model, self.d_model, bias=True), nn.Softplus(beta=10.0)
        )

        self.intensity_layer = nn.Sequential(
            nn.Linear(self.d_model, self.process_dim, bias=True), nn.Softplus(beta=1.0)
        )
    ...
    def state_decay(self, converge_point, start_point, omega, duration_t):
        ...
        cell_t = torch.tanh(
            converge_point
            + (start_point - converge_point) * torch.exp(-omega * duration_t)
        )
        return cell_t
    ...
    def forward(self, seq_dt, seq_types, src_mask):
        ...
        for i in range(self.nLayers):
            x = self.input_sublayer(
                x, lambda _x: self.attention.forward(_x, _x, _x, mask=src_mask)
            )
    ...
    def compute_loss(self, seq_times, seq_onehot_types, n_mc_samples=20):
        ...
        cell_t = self.state_decay(
            self.converge_point, self.start_point, self.omega, dt_seq[:, :, None]
        )
        ...
        intens_at_evs = nn.utils.rnn.pad_sequence(
            intens_at_evs, padding_value=1.0, batch_first=True
        )  # pad with 0 to get rid of the non-events, log1=0
        ...
        taus = torch.rand(n_batch, n_times, 1, n_mc_samples).to(
            device
    ...
    def read_predict(
        ...
        with torch.no_grad():
            ...
            h_t_vals = self.state_decay(
                converge_point, start_point, omega, dt_vals[:, None]
            )
            if print_info:
                print(
                    "last event: time {:.3f} type {:.3f}".format(
                        last_t.item(), last_type.item()
                    )
                )
                print(
                    "next event: time {:.3f} type {:.3f}, in {:.3f}".format(
                        next_t.item(), next_type.item(), next_dt.item()
                    )
                )

            return predict_from_hidden(
                self,
                h_t_vals,
                dt_vals,
                next_dt,
    ...
    def plot_estimated_intensity(
        ...
        seq_times = torch.cat(
            (torch.zeros_like(event_timestamps[:1]), event_timestamps), dim=0
        ).float()  # add 0 to the sequence beginning
        dt_seq = seq_times[1:] - seq_times[:-1]
        ...
        for i in range(self.process_dim):
            intens_at_samples, intens_at_evs = self.intensity_per_type(
                seq_types, dt_seq, sample_times, timestamps[i], type=i
            )
            intens_at_evs_lst.append(intens_at_samples)
            if qqplot is None:
                self._plot_tick_intensity(
                    timestamps[i],
                    sample_times,
                    intens_at_samples,
                    intens_at_evs,
    ...
    def intensity_per_type(self, seq_types, dt_seq, sample_times, timestamps, type):
        ...
        with torch.no_grad():
            ...
            event_types_masked = MaskBatch(
                seq_types[None, :], pad=self.process_dim, device="cpu"
            )
            ...
            cell_t = self.state_decay(
                converge_point, start_point, omega, dt_seq[:, None]
            )  #
            ...
            for t_time in sample_times:
                ...
                if (
                    event_idx < onetype_length - 1
                    and t_time >= timestamps[event_idx + 1]
                ):
                    event_idx += 1
                    # print(omega)
                ...
                event_types_masked = MaskBatch(
                    bbb[None, :], pad=self.process_dim, device="cpu"
                )
                event_types_mask = event_types_masked.src_mask
                ...
                cell_t = self.state_decay(
                    converge_point, start_point, omega, t_time - timestamps[event_idx]
                )  #
